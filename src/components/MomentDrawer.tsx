      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="bg-[var(--bg-desk)] border-t-2 border-amber-500 rounded-t-2xl fixed bottom-0 left-0 right-0 z-50 max-h-[85vh] overflow-y-auto font-mono text-[var(--text-main)] shadow-2xl p-6">
          <div className="max-w-2xl mx-auto space-y-4">
            {/* Drawer Drag Handle */}
            <div className="w-12 h-1.5 bg-[var(--border-receipt)] rounded-full mx-auto mb-2" />

            <div className="flex items-start justify-between border-b border-[var(--border-receipt)] pb-3">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded bg-amber-500/20 border border-amber-500/40 text-amber-500 dark:text-amber-400 flex items-center justify-center">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-amber-500 dark:text-amber-400 uppercase font-bold tracking-wider">
              ...
              <button
                onClick={closeDrawer}
                className="p-1.5 rounded-md text-[var(--text-muted)] hover:text-[var(--text-main)] bg-[var(--bg-desk-secondary)] border border-[var(--border-receipt)]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="bg-[var(--bg-desk-secondary)] p-4 rounded-lg border border-[var(--border-receipt)] space-y-2">
              ...
              {activeMoment && (
                <div className="flex items-center gap-2 text-xs pt-1">
                  <span className="text-[var(--text-muted)]">Connection Strength:</span>
                  <div className="flex-1 bg-[var(--bg-desk)] border border-[var(--border-receipt)] h-2 rounded-full overflow-hidden">
                    <div
            ...
            {selectedReceipt && (
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider flex items-center gap-1">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {activeConnections.length > 0 ? (
                    activeConnections.map((c, i) => {
                      ...
                      return (
                        <div key={i} className="bg-[var(--bg-desk-secondary)] p-3 rounded border border-[var(--border-receipt)] text-xs space-y-1">
                          <div className="flex justify-between font-bold text-amber-500 dark:text-amber-300">
                    ...
                    <div className="text-xs text-[var(--text-muted)] italic bg-[var(--bg-desk-secondary)] p-3 rounded border border-[var(--border-receipt)]">
            ...
            <div className="pt-2 flex justify-end">
              <button
                onClick={closeDrawer}
                className="px-4 py-2 rounded bg-amber-500 text-black font-bold text-xs hover:bg-amber-400 cursor-pointer"
              >
